...
class Patient(object):
    ...
    def update(self):
        ...
        for clear in clearedViruses:
            self.viruses.remove(clear)
        for new in newViruses:
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    averages = [sum(result) / float(numTrials) for result in results]
    
    pylab.plot(averages, label = 'Total Virus Population')
    pylab.title('Simple Virus Simulation for 300 steps')
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        random.random() <= (self.maxBirthProb * (1 - popDensity))):
            for protein in self.resistances.keys():
                if random.random() <= (1 - self.mutProb):
                    mutResistances.update({protein: self.resistances[protein]})
                else:
                    mutResistances.update({protein: not self.resistances[protein]})
            #print mutResistances
            return ResistantVirus(self.maxBirthProb, self.clearProb,\
            mutResistances, self.mutProb)
        else:
            raise NoChildException()
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for trial in range(numTrials):
        ...
        for i in range(150):
            results[i].append(treatedPatient.update())
            resViruses[i].append(treatedPatient.getResistPop(['guttagonol']))
    ...
    averages = [sum(result) / float(numTrials) for result in results]
    averagesVirus = [sum(resis) / float(numTrials) for resis in resViruses]
    
    pylab.plot(averages, label = 'Total Virus Population')
    pylab.plot(averagesVirus, label = 'Resistant Virus Population')
    pylab.title('Simple Virus Simulation for 300 steps')
    pylab.xlabel('Time Steps')
    pylab.ylabel('Average Virus Population')
    pylab.legend()
